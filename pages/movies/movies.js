...
Page({
  ...
  getmovies: function() {
    var that = this;
    wx.request({
      url: "https://movie.douban.com/j/search_subjects?type=movie&page_limit=" + that.data.currentmovies + "&page_start=" + that.data.premovies + "&tag=" + that.data.currenttype,
      header: {
        "Content-Type": "application/text"
      },
      // success: function (res) {
  ...
  towant: function(e) {
    ...
    this.setData({
      ...
      // getApp().movieslist = that.data.movieslist;
    });
    // 从本地存储中获取数据
    ...
    // 当本地存储中没有数据时
    if (wantarr.length == 0) {
      // wx.setStorageSync('Want', wantarr.concat(e.target.dataset.item));
      ...
      for (var i = 0; i < wantarr.length; i++) {
        ...
        if (wantarr[i].id == e.target.dataset.item.id) {
          // filter是为了过滤出id不等的数据成为一个新数组在写入到本地存储中
